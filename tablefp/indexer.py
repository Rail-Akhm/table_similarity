...
from tablefp.hashing import add_ngram_hashes
        ...
        with get_cursor(conn) as cur:
            ...
            while True:
                ...
                if not rows:
                    break
                hashes.append(
                    np.fromiter((h for (h,) in rows), dtype=np.int64, count=len(rows))
                )
    except Exception as e:
        raise RuntimeError(f"hash query failed: {e}") from e

    # Sort and save (chunked numpy arrays instead of a Python-int list)
    hashes_array = np.sort(np.concatenate(hashes))
                ...
                with get_cursor(conn) as cur:
                    ...
                    while True:
                        ...
                        if not rows:
                            break
                        add_ngram_hashes(
                            (v for (v,) in rows), ngram_hashes, n=fc.get("ngram_size", 3)
                        )
            except Exception as e:
                raise RuntimeError(f"n-gram query failed: {e}") from e

            ngrams_array = np.sort(
                np.fromiter(ngram_hashes, dtype=np.int64, count=len(ngram_hashes))
            )
